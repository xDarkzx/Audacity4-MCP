...
- **VST3 realtime parameters revert if the plugin's editor is open.** Write a parameter while the plugin's own GUI window is open and it reports success with the right value, then reverts. Close the editor first and it sticks. This is *not* specific to discrete/list parameters, as this project originally documented — a live session against FabFilter Pro-Q 3's per-band Frequency and Gain (continuous, `recent-commands` log, 2026-09-05) showed them reverting toward the parameter's minimum on re-read. The practical workaround is built into the tools: use `add_realtime_effects` to configure a plugin as it is added (its editor cannot be open yet), or close the editor before `set_effect_parameters`.

  Audacity knows about this: it is issue [audacity/audacity#11892](https://github.com/audacity/audacity/issues/11892), filed by a Muse Group developer, with an open fix in [#11895](https://github.com/audacity/audacity/pull/11895) that replaces the zero-sample `Process(nullptr, nullptr, 0)` flush with a short silent block, for plugin frameworks (DPF, used by most Linux VST3s) that return early when there are no samples.

  Whether that fix covers this case is **not yet established**, and the honest answer is that the root cause here is still open. What has been read out of the source so far: `VST3Wrapper::FlushParameters` does nothing at all when `mActive` is set (its whole body is inside `if (!mActive)`, with an `else` that only clears the caller's `hasChanges` flag); for a realtime effect `mActive` is true only *during playback*, since `RealtimeEffectState` initializes and finalizes the instance on stream start and stop; and `StoreSettings` builds a fresh settings object from `getState()` and `std::swap`s it into place, which discards any `parameterChanges` that have not yet been delivered. Those three together are enough to lose a write, but which one actually fires in this scenario has not been confirmed under instrumentation. Until it is, treat the workaround above as the reliable path.
