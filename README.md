...
Tool count is real, not aspirational — every tool below maps to a command that has been implemented and registered on the C++ side; see [docs/TOOLS.md](docs/TOOLS.md) for the full, current list (156 tools as of this writing, across transport, track, edit, selection, effects, realtime/VST3 effects, generators, labels, project lifecycle, analysis, transcription, and cleanup pipelines).

Don't need all 156 in one session? Set `AUDACITY4_MCP_PROFILE` (e.g. `cleanup`, `editing`, `mastering`, `transcription`, `minimal`) in your MCP client config to load only the tools for one workflow — see [Tool Profiles](docs/TOOLS.md#tool-profiles).
