...
| `add_realtime_effects(track_id, effects)` | **Build a whole chain in one call**, each effect configured as it is added: `[{"effect_id": ..., "parameters": {id: value}}]`. Prefer this for setting up a chain: one call instead of an add plus a write per value, and each effect is configured before the next is added. |
...
| `list_effect_parameters(track_id, index)` | Real, plugin-reported parameters — name, min/max/default/current value, formatted string. Works uniformly across Builtin/VST3/LV2/AudioUnit. **Don't trust `units` or `type` on VST3 plugins**: `units` can carry the plugin's parameter *group* name ("Band 2") instead of a measurement unit, and any quantised control is reported as `Dropdown`. Judge scale from `minValue`/`maxValue` and the name — see [Known Gaps](../README.md#known-gaps). |
| `set_effect_parameter(track_id, index, parameter_id, value)` | Set one parameter to an exact value. Works with the plugin's editor open or closed, and the change is drawn in the plugin's own GUI as it lands. **VST3 min/max is typically normalized 0–1** for some plugins but real display units (e.g. log2-Hz, dB) for others, and the curve between them is rarely linear — set a value, read the returned string, adjust. Two probes is usually enough to hit an exact target (`0.1855` → `10.00 ms`). |
| `set_effect_parameters(track_id, index, parameters)` | **Set several parameters in one commit** — `{parameter_id: value}`. Prefer this whenever more than one value is involved: the whole batch is written before anything is flushed, so the plugin is never briefly half-configured in a way you can hear while it processes (an EQ band enabled before its frequency is set, say), and it is one settings commit rather than one per value. Nothing is written if any entry is invalid. Works with the plugin's editor open. Note that each value is currently its own settings commit rather than one for the batch, so a chain processing audio can be heard passing through intermediate states. |
