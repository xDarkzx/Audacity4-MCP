...
| `set_effect_parameter(track_id, index, parameter_id, value)` | Set one parameter to an exact value. **VST3 min/max is typically normalized 0–1** for some plugins but real display units (e.g. log2-Hz, dB) for others — re-read `currentValueString` after setting rather than assuming the input scale. **Close the plugin's editor first:** a write made while its GUI is open reports success and then reverts ([audacity/audacity#11892](https://github.com/audacity/audacity/issues/11892)), for continuous parameters too, not just discrete ones. Prefer `add_realtime_effects` or `set_effect_parameters` — see [Known Gaps](../README.md#known-gaps). |
